...
def check_email_reputation(email):
    ...
    try:
        ...
        if disposable:
            log_email_activity(email, "Disposable", "Blocked", "Temporary Email Detected")
            return f"🚨 **Temporary Email Detected! BLOCKED!**\n❌ **{email} is Disposable.**"

        log_email_activity(email, "Checked", "Allowed", "Reputation Check Passed")
        return f"""
...
option = st.radio("🔍 What do you want to check?", ("Email Breach", "Email Reputation", "Website Safety", "Live Logs"))
